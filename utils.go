...
func attrToFileInfo(inode uint32, attr []byte) (*os.FileInfo) {
    ...
    case TYPE_DIRECTORY, TYPE_SYMLINK, TYPE_FILE:
        switch type_ {
        case TYPE_DIRECTORY: fi.Mode = uint32(mode&07777) | S_IFDIR
        case TYPE_SYMLINK: fi.Mode = uint32(mode&07777) | S_IFLNK
        case TYPE_FILE: fi.Mode = uint32(mode&07777) | S_IFREG
        }
        var length int64
        read(r, &length)
        fi.Size = length
